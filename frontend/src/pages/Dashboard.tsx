...
export default function Dashboard() {
  ...
  const statCards = [
    ...
    { label: t('dashboard.trialing'), value: stats?.by_plan_state?.trial ?? 0, icon: Clock, color: 'text-amber-400' },
    ...
    <div className="space-y-2">
      ...
      {values && Object.keys(values).length > 0 ? Object.entries(values).map(([status, count]) => (
        <div key={status} className="flex items-center justify-between rounded-lg border border-[var(--border-soft)] bg-[var(--bg-pane)]/45 px-3 py-2.5">
          <Badge variant={STATUS_VARIANT[status] || 'secondary'}>{STATUS_LABELS[locale][status] || STATUS_LABELS.zh[status] || status}</Badge>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.45fr)_minmax(300px,0.55fr)]">
        ...
        <Card>
          <CardHeader><CardTitle>{t('dashboard.desktopState')}</CardTitle></CardHeader>
          <CardContent className="space-y-3">
                    ...
                    <div>
                      ...
                      <div className="mt-1 text-xs leading-5 text-[var(--text-muted)]">
                        {state?.available === false
                          ...
                          : (state?.ready_label || state?.status_label || t('dashboard.desktopHint'))}
                      </div>
                    </div>
                    <Badge variant={state?.ready ? 'success' : 'secondary'}>
                      {state?.ready ? t('dashboard.ready') : t('dashboard.standby')}
                    </Badge>
                  </div>
