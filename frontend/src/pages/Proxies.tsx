...
export default function Proxies() {
  ...
  const metricCards = [
    { label: t('proxies.stats.total', '代理数'), value: proxies.length, icon: Globe2, tone: 'text-[var(--accent)]' },
    { label: t('proxies.stats.active', '启用'), value: activeCount, icon: ShieldCheck, tone: 'text-emerald-400' },
    { label: t('proxies.stats.success', '成功次数'), value: totalSuccess, icon: Activity, tone: 'text-[var(--accent)]' },
    { label: t('proxies.stats.fail', '失败次数'), value: totalFail, icon: CircleOff, tone: 'text-red-400' },
  ]
    ...
    <div className="space-y-4">
      <Card className="overflow-hidden p-2.5">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="flex flex-wrap items-center gap-2">
            <div className="text-sm font-semibold text-[var(--text-primary)]">{t('proxies.title', '代理')}</div>
            <Badge variant="default">{t('proxies.total_label', '总量')} {proxies.length}</Badge>
            <Badge variant="secondary">{t('proxies.active_label', '活跃')} {activeCount}</Badge>
          </div>
          <Button variant="outline" size="sm" onClick={check} disabled={checking}>
            <RefreshCw className={`h-4 w-4 mr-1.5 ${checking ? 'animate-spin' : ''}`} />
            {t('proxies.check_btn', '检测全部')}
          </Button>
        </div>
      </Card>
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,330px)_minmax(0,1fr)]">
        <Card className="bg-[var(--bg-pane)]/60">
          <div className="space-y-4">
            <div>
              <div className="text-[11px] uppercase tracking-[0.18em] text-[var(--text-muted)]">{t('proxies.add_label', '新增')}</div>
              <div className="mt-1 text-sm font-medium text-[var(--text-primary)]">{t('proxies.add_desc', '添加代理或批量导入')}</div>
            </div>
            <textarea
              value={newProxy}
            ...
            <input
              ...
              placeholder={t('proxies.region_placeholder', '地区标签 (如 US, SG)')}
            ...
            <div className="rounded-lg border border-[var(--border-soft)] bg-[var(--bg-pane)]/45 px-3.5 py-2.5 text-xs leading-5 text-[var(--text-secondary)]">
              {t('proxies.add_hint', '支持单条代理直接录入，也支持多行批量导入。地区标签会一起写入，用于后续筛选和出入口识别。')}
            </div>
            <Button onClick={add} className="w-full">
              <Plus className="h-4 w-4 mr-1.5" />
              {t('proxies.add_btn', '添加到代理池')}
            </Button>
          </div>
        </Card>
        ...
        <Card className="overflow-hidden p-0">
          <div className="border-b border-[var(--border)] px-4 py-3 text-sm font-medium text-[var(--text-primary)]">
            {t('proxies.list_title', '代理列表')}
          </div>
        <div className="glass-table-wrap">
        <table className="w-full min-w-[760px] text-sm">
          <thead>
            <tr className="border-b border-[var(--border)] text-[var(--text-muted)]">
              <th className="px-4 py-2.5 text-left">{t('proxies.table.url', '代理地址')}</th>
              <th className="px-4 py-2.5 text-left">{t('proxies.table.region', '地区')}</th>
              <th className="px-4 py-2.5 text-left">{t('proxies.table.stats', '成功/失败')}</th>
              <th className="px-4 py-2.5 text-left">{t('proxies.table.status', '状态')}</th>
              <th className="px-4 py-2.5 text-left">{t('proxies.table.actions', '操作')}</th>
            </tr>
          </thead>
          <tbody>
            {proxies.length === 0 && (
              <tr>
                <td colSpan={5} className="px-4 py-8">
                  <div className="empty-state-panel">{t('proxies.empty', '当前代理池为空，可以先从左侧输入一个或批量导入。')}</div>
                </td>
              </tr>
            )}
            {proxies.map(p => (
              <tr key={p.id} className="border-b border-[var(--border)]/40 hover:bg-[var(--bg-hover)]/70">
                ...
                <td className="px-4 py-2.5">
                  <Badge variant={p.is_active ? 'success' : 'danger'}>
                    {p.is_active ? t('proxies.row.active', '活跃') : t('proxies.row.inactive', '禁用')}
                  </Badge>
                </td>
                <td className="px-4 py-2.5">
                  <div className="flex items-center gap-2">
                    <button onClick={() => toggle(p.id)} className="table-action-btn">
                      ...
                      {p.is_active ? t('proxies.row.disable_btn', '停用') : t('proxies.row.enable_btn', '启用')}
                    </button>
                    <button onClick={() => del(p.id)} className="table-action-btn table-action-btn-danger">
                      <Trash2 className="mr-1.5 h-4 w-4" />
                      {t('proxies.row.delete_btn', '删除')}
                    </button>
                  </div>
                </td>
              </tr>
