...
export default function Proxies() {
  ...
  const metricCards = [
    { label: t('proxies.proxyCount'), value: proxies.length, icon: Globe2, tone: 'text-[var(--accent)]' },
    { label: t('proxies.activeCount'), value: activeCount, icon: ShieldCheck, tone: 'text-emerald-400' },
    { label: t('proxies.successCount'), value: totalSuccess, icon: Activity, tone: 'text-[var(--accent)]' },
    { label: t('proxies.failCount'), value: totalFail, icon: CircleOff, tone: 'text-red-400' },
  ]
    ...
    <div className="space-y-4">
      <Card className="overflow-hidden p-2.5">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge variant="default">{t('proxies.totalBadge', { count: proxies.length })}</Badge>
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,330px)_minmax(0,1fr)]">
        <Card className="bg-[var(--bg-pane)]/60">
          <div className="space-y-4">
            <div>
              <div className="text-[11px] uppercase tracking-[0.18em] text-[var(--text-muted)]">{t('proxies.addSection')}</div>
              <div className="mt-1 text-sm font-medium text-[var(--text-primary)]">{t('proxies.addDescription')}</div>
            </div>
            <textarea
              value={newProxy}
        ...
        <table className="w-full min-w-[760px] text-sm">
          <thead>
            <tr className="border-b border-[var(--border)] text-[var(--text-muted)]">
              ...
              <th className="px-4 py-2.5 text-left">{t('proxies.status')}</th>
              <th className="px-4 py-2.5 text-left">{t('proxies.actions')}</th>
            </tr>
          </thead>
          <tbody>
            {proxies.length === 0 && (
              ...
              <tr key={p.id} className="border-b border-[var(--border)]/40 hover:bg-[var(--bg-hover)]/70">
                ...
                <td className="px-4 py-2.5">
                  <Badge variant={p.is_active ? 'success' : 'danger'}>
                    {p.is_active ? t('proxies.active') : t('proxies.disabled')}
                  </Badge>
                </td>
                <td className="px-4 py-2.5">
                  <div className="flex items-center gap-2">
                    <button onClick={() => toggle(p.id)} className="table-action-btn">
                      ...
                      {p.is_active ? t('proxies.disable') : t('proxies.enable')}
                    </button>
