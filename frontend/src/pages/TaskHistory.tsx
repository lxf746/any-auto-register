...
export default function TaskHistory() {
  ...
  const metricCards = [
    { label: t('taskHistory.taskCount'), value: tasks.length, icon: Activity, tone: 'text-[var(--accent)]' },
    { label: t('taskHistory.success'), value: succeeded, icon: CheckCircle2, tone: 'text-emerald-500' },
    { label: t('taskHistory.failure'), value: failed, icon: AlertTriangle, tone: 'text-red-500' },
    { label: t('taskHistory.inProgress'), value: running, icon: Clock3, tone: 'text-amber-500' },
  ]
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] overflow-hidden">
        <div className="flex items-center gap-3 border-b border-[var(--border)] px-4 py-2.5">
          <span className="text-sm font-medium text-[var(--text-primary)]">{t('taskHistory.recentTasks')}</span>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              ...
              >
                ...
                <option value="running">{getTaskStatusText('running', locale)}</option>
                <option value="succeeded">{getTaskStatusText('succeeded', locale)}</option>
                <option value="failed">{getTaskStatusText('failed', locale)}</option>
                <option value="cancelled">{getTaskStatusText('cancelled', locale)}</option>
                <option value="interrupted">{getTaskStatusText('interrupted', locale)}</option>
              </select>
              <ChevronDown className="pointer-events-none absolute right-2 top-1/2 h-3 w-3 -translate-y-1/2 text-[var(--text-muted)]" />
            </div>
            {(platform || status) && (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[var(--border)] bg-[var(--bg-pane)]">
                <th className="px-4 py-2.5 text-left text-xs font-medium text-[var(--text-muted)]">{t('taskHistory.time')}</th>
                <th className="px-4 py-2.5 text-left text-xs font-medium text-[var(--text-muted)]">{t('taskHistory.taskId')}</th>
                <th className="px-4 py-2.5 text-left text-xs font-medium text-[var(--text-muted)]">{t('taskHistory.platform')}</th>
                <th className="px-4 py-2.5 text-left text-xs font-medium text-[var(--text-muted)]">{t('taskHistory.status')}</th>
                <th className="px-4 py-2.5 text-left text-xs font-medium text-[var(--text-muted)]">{t('taskHistory.progress')}</th>
                <th className="px-4 py-2.5 text-left text-xs font-medium text-[var(--text-muted)]">{t('taskHistory.successFailure')}</th>
                <th className="px-4 py-2.5 text-left text-xs font-medium text-[var(--text-muted)]">{t('taskHistory.error')}</th>
              </tr>
            </thead>
            <tbody>
              {tasks.length === 0 && (
