    ...
    <div className="flex h-full flex-col gap-4">
      <div className="grid gap-3 md:grid-cols-3">
        <div className={`rounded-2xl border px-4 py-3 ${statusTone}`}>
          <div className="text-[11px] uppercase tracking-[0.18em] opacity-70">{t('history.table.status', '状态')}</div>
          <div className="mt-1 text-sm font-semibold">{getTaskStatusText(currentStatus, t)}</div>
        </div>
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg-hover)] px-4 py-3">
          <div className="text-[11px] uppercase tracking-[0.18em] text-[var(--text-muted)]">{t('history.table.progress', '进度')}</div>
          <div className="mt-1 text-sm font-semibold text-[var(--text-primary)]">{progress.label || task?.progress || '0/0'}</div>
        </div>
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg-hover)] px-4 py-3">
          <div className="text-[11px] uppercase tracking-[0.18em] text-[var(--text-muted)]">{t('history.table.events', '日志')}</div>
          <div className="mt-1 text-sm font-semibold text-[var(--text-primary)]">{t('history.events_count', '{{count}} 条日志', { count: lines.length })}</div>
        </div>
      </div>
      ...
      {errorText ? (
        <div className="rounded-2xl border border-red-400/35 bg-red-500/10 px-4 py-3 text-sm text-red-100">
          <div className="mb-1 font-semibold">{t('history.error_reason', '失败原因')}</div>
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          <div className="text-[11px] uppercase tracking-[0.18em] text-[var(--text-muted)]">{t('history.live_log', 'Live Log')}</div>
          <div className="mt-1 text-sm font-medium text-[var(--text-primary)]">{t('history.live_log_desc', '实时执行日志')}</div>
        </div>
        <button
          type="button"
          onClick={copyLogs}
