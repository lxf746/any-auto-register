...
export default function UpdateBanner() {
  ...
  const currentVersion = info.current === 'dev' ? 'dev' : info.current
    ...
    <div className="mb-3 flex items-center justify-between gap-3 rounded-lg border border-[var(--accent-edge)] bg-[var(--accent-soft)] px-4 py-2.5 text-sm">
      <div className="flex items-center gap-2 min-w-0">
        ...
        <span className="text-[var(--text-primary)] truncate">
          {t('update.available', '有新版本')} <b>v{info.latest.tag}</b> {t('update.downloadable', '可下载')}（{t('update.current', '当前')} v{currentVersion}）
        </span>
      </div>
      <div className="flex items-center gap-2 shrink-0">
        ...
        <button
          ...
          aria-label={t('update.dismiss', '忽略')}
