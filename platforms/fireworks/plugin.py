"""fireworks.ai platform plugin

Auth model: Email/password registration with email verification.
  - Signup via browser (email/password)
  - Email verification link sent by Fireworks
  - After verification: login → create API key
"""
...
class FireworksPlatform(BasePlatform):
    ...
    def build_protocol_mailbox_adapter(self):
        def _build_worker(ctx, artifacts):
            from platforms.fireworks.protocol_mailbox import FireworksProtocolMailboxWorker
            return FireworksProtocolMailboxWorker(
                proxy=ctx.proxy,
                log_fn=ctx.log,
            )
        ...
        return ProtocolMailboxAdapter(
            ...
            link_spec=LinkSpec(
                keyword="verify",
                wait_message="Waiting for Fireworks verification email...",
    ...
    def check_valid(self, account: Account) -> bool:
        ...
        try:
            ...
            result = FireworksRegister().check_api_key_valid(api_key)
